...
Item {
  ...
  // The layer surface provides a generous pointer target while the island
  // chrome remains visually compact inside it.
  readonly property int barSize: (vertical ? Style.bar.sizeVertical : Style.bar.sizeHorizontal) + Style.space(10)
  readonly property int islandInset: Style.space(4)
  readonly property int islandHeight: Math.max(1, barSize - islandInset * 2)
  readonly property int islandPadding: Style.space(5)
  ...
  function isSecondaryWidget(entry) {
    return secondaryWidgetIds.indexOf(entryId(entry)) !== -1
  }

  function primaryEntries(entries) {
    return entries.filter(function(entry) { return !root.isSecondaryWidget(entry) })
  }

  function secondaryEntries(entries) {
    return entries.filter(function(entry) { return root.isSecondaryWidget(entry) })
  }

  function setStatusHovered(hovered) {
    statusHoverCount = Math.max(0, statusHoverCount + (hovered ? 1 : -1))
  }
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.vertical ? (root.transparent ? "transparent" : root.background) : "transparent"
    ...
    Component {
      ...
      Item {
        ...
        BorderSurface {
          ...
          HoverHandler {
            id: statusHover
            onHoveredChanged: root.setStatusHovered(hovered)
            Component.onDestruction: if (hovered) root.setStatusHovered(false)
          }
          ...
          Row {
            ...
            Item {
              width: root.statusRevealed ? secondaryModules.width : 0
              height: root.barSize
              clip: true
              ...
              ModuleList {
                id: secondaryModules
                entries: root.secondaryEntries(root.layoutEntries("right"))
                region: "right"
                anchors.right: parent.right
                anchors.verticalCenter: parent.verticalCenter
                opacity: root.statusRevealed ? 1 : 0
            ...
            ModuleList {
              entries: root.primaryEntries(root.layoutEntries("right"))
              region: "right"
            }
          }
        }
